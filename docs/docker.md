...
```bash
docker build -t athena-tech-adapter .
```
...
```bash
docker run -d --name athena-ta-container -p 8888:8888 athena-tech-adapter
```
...
```bash
docker run -d --name athena-ta-container \
-e OTEL_EXPORTER_OTLP_ENDPOINT=<CollectorURL> \
-e OTEL_METRICS_EXPORTER=otlp \
-e OTEL_SERVICE_NAME=athena-tech-adapter \
-e OTEL_TRACES_EXPORTER=otlp \
-p 8888:8888 athena-tech-adapter
```
